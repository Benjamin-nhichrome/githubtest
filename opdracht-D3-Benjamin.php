...
<body>
    ...
    if (isset($_GET['move_id'])) {
        $id = htmlspecialchars($_GET['move_id']);
        $sql = "SELECT woonplaats FROM klanten WHERE klant_id=?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("i", $id);
        $stmt->execute();
        $stmt->bind_result($woonplaats);
        $stmt->fetch();
        $stmt->close();

        if ($woonplaats !== "Amstelveen") {
            $nieuweWoonplaats = "Amstelveen";
        } else {
            $nieuweWoonplaats = "Arnhem"; 
        }

        $sql = "UPDATE klanten SET woonplaats=? WHERE klant_id=?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("si", $nieuweWoonplaats, $id);
        $stmt->execute();
        $stmt->close();

        echo "<p>Woonplaats van klant met ID $id is bijgewerkt naar $nieuweWoonplaats.</p>";
    }
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            echo "<td>";
            echo "<a href='database-connectie-Benjamin.php?verwijder_id=" . $row["klant_id"] . "' class='btn btn-verwijder'>Verwijder</a> ";
            echo "<a href='database-connectie-Benjamin.php?move_id=" . $row["klant_id"] . "' class='btn btn-move'>Move</a>";
            echo "</td>";
            echo "</tr>";
        }
    } else {
